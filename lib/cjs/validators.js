  ...
  call(value) {
    ...
    if (compare.startOf('day') < min.startOf('day')) {
      throw new Error(
        JSON.stringify({
          code: this.code,
          message: `Please enter a date greater than ${luxon_1.DateTime.fromJSDate(min).toFormat(
            'D',
          )}`,
        }),
      )
...
class MaxDateValidator extends Validator {
  ...
  call(value) {
    ...
    if (
      luxon_1.DateTime.fromJSDate(value).startOf('day') >
      luxon_1.DateTime.fromJSDate(this.max).startOf('day')
    ) {
      throw new Error(
        JSON.stringify({
          code: this.code,
          message: `Please enter a date greater than ${luxon_1.DateTime.fromJSDate(
            this.max,
          ).toFormat('D')}`,
        }),
      )
