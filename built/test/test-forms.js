...
describe('Form Model', () => {
    ...
    UserAddressForm.street = new FormField({ validators: [] });
    UserAddressForm.city = new FormField({ validators: [new MinLengthValidator({ minLength: 5 })] });
    class UserForm extends Form {
    }
    UserForm.firstName = new FormField({ validators: [new MinLengthValidator({ minLength: 5 })] });
    /*     static address = new FormArray({
      name: 'address',
      groups: [new UserAddressForm()],
    }) */
    UserForm.password = new FormField({ validators: [new MinLengthValidator({ minLength: 5 })] });
    UserForm.confirmPassword = new FormField({
    ...
    describe('#constructor()', () => {
        ...
        it('should  add a validator on the fly to confirmPassword', () => {
            userForm.addValidator('confirmPassword', new MustMatchValidator({
                message: 'This Field Must Match The Password Field',
                matcher: userForm.field['password'],
            }));
            assert.equal(userForm.field['confirmPassword'].validators[1] instanceof MustMatchValidator, true);
        });
